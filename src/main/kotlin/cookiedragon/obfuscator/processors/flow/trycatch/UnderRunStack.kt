 ...
 */
object UnderRunStack: IClassProcessor {
	override fun process(classes: MutableCollection<ClassNode>, passThrough: MutableMap<String, ByteArray>) {
		for (classNode in classes) {
			if (!CObfuscator.isExcluded(classNode) && classNode.methods.size > 0){// && CObfuscator.randomWeight(4)) {
				for (method in classNode.methods) {
					...
					val list = InsnList()
						.also {
							...
							it.add(MethodInsnNode(INVOKESTATIC, System::class.internalName, "currentTimeMillis", "()J"))
							it.add(InsnNode(L2I))
