...
object MethodParameterObfuscator: IClassProcessor {
    ...
    inline fun mnToStr(insn: MethodInsnNode, add: Boolean = false)  = insn.owner + "." + insn.name + if (!add) insn.desc else insn.desc.replace(")", "I)")
    inline fun mnToStr(hn: Handle, add: Boolean = false)  = hn.owner + "." + hn.name + if (!add) hn.desc else hn.desc.replace(")", "I)")
