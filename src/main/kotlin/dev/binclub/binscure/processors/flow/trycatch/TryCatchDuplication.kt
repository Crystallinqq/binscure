					...
					if (!mn.name.startsWith("<")) {
						mn.instructions?.let { insns ->
							if (insns.size() > 30) {
								...
								val tryCatchBlocks: List<TryCatchBlockNode> = mn.tryCatchBlocks ?: Collections.emptyList()
								...
								if (blocks.size > 2) {
									...
									for (i in 0 until addTcbs) {
										...
										val length = random.nextInt(mn.instructions.size() - 1 - startIndex).coerceAtLeast(minTcbLength)
										val endIndex = startIndex + length
										mn.instructions.insert(mn.instructions.get(endIndex), endLabel)
										
										println("TCB $startIndex -> $endIndex")
										if (endIndex <= startIndex) error("")
										
										val tcb = TryCatchBlockNode(startLabel, endLabel, handlerLabel, randomThrowable())
										mn.tryCatchBlocks.add(tcb)
									}
									
									mn.localVariables = null
									println("Adding to $cn.$mn")
									verifyMethodNode(mn)
									injected = true
								}
							}
