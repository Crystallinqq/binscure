...
fun calculateHeightFromInsn(stack: Stack<Type>, abstractInsnNode: AbstractInsnNode, registers: MutableMap<Int, Type>, tryCatchBlockNodes: Collection<TryCatchBlockNode>, out: MutableMap<AbstractInsnNode, MutableList<Stack<Type>>>) {
		...
		when (insn) {
			...
			is LdcInsnNode -> {
				when (val cst = insn.cst) {
					...
					is Type -> {
						when (cst.sort) {
							Type.OBJECT -> stack.push(Type.getType("java/lang/Class"))
							Type.METHOD -> stack.push(Type.getType("java/lang/invoke/MethodType"))
							else -> stack.push(Type.getObjectType(cst.descriptor))
						}
					}
					is Handle -> stack.push(Type.getType("java/lang/invoke/MethodHandle"))
			...
			is LookupSwitchInsnNode -> {
				...
				for (label in insn.labels) {
					try {
						calculateHeightFromInsn(stack.clone() as Stack<Type>, label, registers, tryCatchBlockNodes, out)
					} catch (e: StackOverflowError) {
						IllegalStateException("Found infinite loop while following stack", e).printStackTrace()
					}
				}
				try {
					calculateHeightFromInsn(stack.clone() as Stack<Type>, insn.dflt, registers, tryCatchBlockNodes, out)
				} catch (e: StackOverflowError) {
					IllegalStateException("Found infinite loop while following stack", e).printStackTrace()
				}
...
fun calculateStackHeight(insnList: InsnList, tryCatchBlockNodes: Collection<TryCatchBlockNode>): Map<AbstractInsnNode, MutableList<Stack<Type>>> {
	val out = hashMapOf<AbstractInsnNode, MutableList<Stack<Type>>>()
	val insn = insnList.first ?: return out
	
	val registers = hashMapOf<Int, Type>()
	try {
...
val NULL_TYPE = Type.getObjectType("null")
